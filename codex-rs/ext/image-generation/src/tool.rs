...
async fn save_image_generation_result(
    output_root: Option<&AbsolutePathBuf>,
    environment: Option<&ToolEnvironment<'_>>,
    metadata: &ImageArtifactMetadata,
    call_id: &str,
    result: &str,
) -> Option<AbsolutePathBuf> {
    let (output_dir, save_result) = match output_root {
        Some(output_root) => {
            ...
            let output_dir = path.parent().unwrap_or_else(|| output_root.clone());
            let save_result: io::Result<AbsolutePathBuf> = async {
                let bytes = decode_image_generation_result(result, metadata)?;
...
fn decode_image_generation_result(
    result: &str,
    metadata: &ImageArtifactMetadata,
) -> io::Result<Vec<u8>> {
    let result = result.trim();
    ...
    embed_png_metadata(bytes, metadata)
}
