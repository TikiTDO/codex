...
impl TurnRequestProcessor {
    ...
    async fn thread_attention_inner(
        ...
        let submission = thread
            .start_turn_if_idle(
                TurnInputRequest::new(TurnInput::ResponseItem(marker))
                    .with_trace(self.request_trace_context(request_id).await),
            )
            .await
            .map_err(|error| {
                let error = internal_error(format!("failed to admit thread attention: {error}"));
                self.track_error_response(request_id, &error, /*error_type*/ None);
                error
            })?;
        match submission {
            StartIfIdleSubmission::Started { .. } => Ok(ThreadAttentionResponse::Started {}),
            StartIfIdleSubmission::NotSubmitted {
                reason: NotSubmittedReason::PendingTriggerTurn,
            } => Ok(ThreadAttentionResponse::Held {
                reason: ThreadAttentionHeldReason::PendingTriggerTurn,
            }),
            StartIfIdleSubmission::NotSubmitted {
                reason: NotSubmittedReason::PlanMode,
            } => Ok(ThreadAttentionResponse::Held {
                reason: ThreadAttentionHeldReason::PlanMode,
            }),
            StartIfIdleSubmission::NotSubmitted {
            ...
            StartIfIdleSubmission::NotSubmitted { reason } => {
                let error = internal_error(format!(
                    "Core declined thread attention with an invalid idle-start reason: {reason:?}"
                ));
                self.track_error_response(request_id, &error, /*error_type*/ None);
                Err(error)
            }
